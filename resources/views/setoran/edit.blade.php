        ...
        <form action="{{ route('setoran.update', $setoran->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="date" name="tanggal" id="tanggal" class="w-full border rounded px-3 py-2"
                    value="{{ $setoran->tanggal }}" required>
            </div>

            <!-- Jenis Sampah -->
            <div class="mb-4">
                <label for="jenis_sampah" class="block font-semibold">Jenis Sampah</label>
                <input type="text" name="jenis_sampah" id="jenis_sampah" class="w-full border rounded px-3 py-2"
                    value="{{ $setoran->jenis_sampah }}" required>
            </div>

            <!-- Item Sampah -->
            <div class="mb-4">
                <label for="item_sampah" class="block font-semibold">Item Sampah</label>
                <input type="text" name="item_sampah" id="item_sampah" class="w-full border rounded px-3 py-2"
                    value="{{ $setoran->item_sampah }}" required>
            </div>

            <!-- Berat -->
            <div class="mb-4">
                <label for="berat" class="block font-semibold">Berat (Kg)</label>
                <input type="number" name="berat" id="berat" class="w-full border rounded px-3 py-2" step="0.1"
                    min="0" value="{{ $setoran->berat }}" required>
            </div>

            <!-- Harga per Kg -->
            <div class="mb-4">
                <label for="harga_per_kg" class="block font-semibold">Harga per Kg (Rp)</label>
                <input type="number" name="harga_per_kg" id="harga_per_kg" class="w-full border rounded px-3 py-2"
                    step="100" min="0" value="{{ $setoran->harga_per_kg }}" required>
            </div>

            <!-- Persentase Nasabah -->
            <div class="mb-4">
                <label for="persentase_nasabah" class="block font-semibold">Persentase untuk Nasabah (%)</label>
                <input type="number" name="persentase_nasabah" id="persentase_nasabah"
                    class="w-full border rounded px-3 py-2" min="1" max="100"
                    value="{{ $setoran->persentase_nasabah }}" required>
            </div>

            <!-- Poin -->
            <div class="mb-4">
                <label for="poin" class="block font-semibold">Poin</label>
                <input type="number" name="poin" id="poin" class="w-full border rounded px-3 py-2" min="0"
                    value="{{ $setoran->poin }}" required>
            </div>

            <button type="submit"
                class="bg-green-700 hover:bg-green-800 text-white px-4 py-2 rounded font-semibold w-full">
                Update Setoran
            </button>
        </form>
    </div>
